...
export function CompanyCategoryFilter() {
  // const globalFilters = useSearchStore((s) => s.categoryFilter);
  // useEffect(() => {
  ...
  //global setter
  const setCategoryFilter = useSearchStore((state) => state.setCategoryFilter);
  ...
  const handleAddCategory = (category: string) => {
    if (!activeCondition || !category.trim()) return;

    const inAndOr =
      filters.and.includes(category) || filters.or.includes(category);
    const inNone = filters.none.includes(category);
    ...
    setFilters((prev) => ({
      ...prev,
      [activeCondition]: prev[activeCondition].includes(category)
        ? prev[activeCondition] // no duplicates
        : [...prev[activeCondition], category],
    }));
  };
  ...
  const handleRemoveCategory = (
    ...
    setFilters((prev) => ({
      ...prev,
      [condition]: prev[condition].filter((c) => c !== category),
    }));
  };
            ...
            >
              {filters[cond].map((category) => (
                ...
                >
                  {category}
                  <button
                    onClick={() => handleRemoveCategory(cond, category)}
